...
seed = np.int64(seed_file[-1].astype(np.int64))
...
for sample_size in sample_sizes:
    ...
    ks_scipy[index], p_scipy[index] = kstest(x_drawn, 'norm')
    ks[index], p[index] = ks_test(x_drawn, cdf_gaussian, args=[])

    index += 1

fig, (ax2, ax1) = plt.subplots(2, 1)
ax1.plot(sample_sizes, ks_scipy, 'x', color='grey', label='Scipy KS-test')
ax1.plot(sample_sizes, ks, '.', color='k', label='My KS-test')
ax1.set_ylim(1e-3, 1)
...
ax2.plot(sample_sizes, p_scipy, c='grey', linestyle='--', label='Scipy probability')
